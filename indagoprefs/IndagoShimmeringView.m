...
@implementation IndagoShimmeringView
- (void)startShimmering {
    CGColorRef lightColor = [UIColor colorWithWhite:0.0 alpha:0.1].CGColor;
    CGColorRef darkColor = [UIColor blackColor].CGColor;

    CAGradientLayer *gradient = [CAGradientLayer layer];
    gradient.colors = @[(id)darkColor, (id)lightColor, (id)darkColor];
    gradient.frame = CGRectMake(-self.bounds.size.width, 0.0, self.bounds.size.width * 3, self.bounds.size.height);
